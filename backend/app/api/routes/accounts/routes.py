...
@router.post("/{account_id}/billing/contract-acceptance", response_model=ContractAcceptanceResponse)
def record_contract_acceptance(
    account_id: int,
    body: ContractAcceptanceRequest,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
) -> ContractAcceptanceResponse:
    """Persist checkout contract signature before payment (audit trail)."""
    require_account_member(account_id, db, user)
    b64 = _strip_data_url_base64(body.signaturePngBase64)
    if len(b64) < 32:
        raise HTTPException(status_code=400, detail="invalid_signature")
    row = BillingContractAcceptance(
        account_id=account_id,
        user_id=user.id,
        payment_action_id=(body.paymentActionId or "").strip()[:255],
        signature_png_base64=b64[:4_000_000],
    )
    db.add(row)
    db.commit()
    db.refresh(row)
    ...
    return ContractAcceptanceResponse(id=row.id, createdAt=created)
...
def list_accounts(
    ...
    with_site = {
        aid
        for (aid,) in db.query(AccountSiteConfig.account_id).distinct().all()
    }
    return [
        AccountOut(id=a.id, name=a.name, hasMeta=a.id in with_meta, hasGoogle=a.id in with_google, hasSite=a.id in with_site)
        for a in rows
    ]
