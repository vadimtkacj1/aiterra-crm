"""
Fake «hosted checkout» HTML + DB lookup — used only when Z-Credit Web Checkout key is unset.

This is not a payment processor; it lets QA click through the same URLs as production mocks.
"""
...
def require_zcredit_mock_mode() -> None:
    if (settings.zcredit_api_key or "").strip():
        raise HTTPException(status_code=404, detail="not_found")


class MockPaymentData(NamedTuple):
    amount: float | None
    currency: str
    description: str
    account_id: int
...
def find_mock_payment_data(db: Session, doc_id: str) -> MockPaymentData | None:
    if doc_id.startswith("ins_"):
        ...
        if ins:
            return MockPaymentData(
                ins.amount,
                ins.currency or "ILS",
                ins.description or "Invoice",
                ins.account_id,
                doc_id,
                ins.line_items_json,
                ins.charge_type or "one_time",
            )
        return None
...
def build_zcredit_hosted_mock_html(*, data: MockPaymentData, confirm_url: str) -> str:
    ...
    <div class="inner">
      ...
      <div class="panel">
        ...
        <div class="consent">
          ...
          <label for="agree">אני מאשר/ת את הסכום המוצג ומסכים/ה להפנייה לעמוד תשלום מאובטח להשלמת העסקה. <a href="/api/cancel-policy" target="_blank" rel="noopener noreferrer">מדיניות ביטולים</a> ו <a href="/api/privacy-policy" target="_blank" rel="noopener noreferrer">מדיניות פרטיות</a>.</label>
        </div>
...
def apply_mock_hosted_confirm(db: Session, doc_id: str, *, success: bool) -> None:
    """Update billing instruction after mock «payment» (no Z-Credit call)."""
