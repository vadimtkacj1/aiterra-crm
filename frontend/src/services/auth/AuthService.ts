...
export class AuthService implements IAuthService {
  ...
  async listUsers(): Promise<User[]> {
    ...
    return raw.map((row): User => {
      ...
      const accountIdRaw = r.accountId ?? r.account_id;
      const accountId =
        typeof accountIdRaw === "number" && Number.isFinite(accountIdRaw) ? accountIdRaw : null;
      return { id, email, displayName, role, accountId: accountId ?? undefined };
    });
  }
