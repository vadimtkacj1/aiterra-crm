...
Locator: getByRole('button', { name: 'OK' }).or(getByRole('button', { name: 'Yes' })).first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: 'OK' }).or(getByRole('button', { name: 'Yes' })).first()
...
- main:
  ...
  - text: Manage users and businesses. Only administrators can access this page.
  - img "team"
  - text: All users
  - img "search"
  - textbox "Search"
  - button "user-add Create account":
    - img "user-add"
    - text: Create account
  - table:
    - rowgroup:
      - row "Select all Email Display name Role Actions":
    ...
    - rowgroup:
      - row "client@test.com Client User User edit lock delete":
        - cell:
          - checkbox
        - cell "client@test.com"
        - cell "Client User"
        - cell "User"
        - cell "edit lock delete":
          - button "edit":
            - img "edit"
          - button "lock":
            - img "lock"
...
- dialog "Delete user?":
  ...
  - text: Delete user? Are you sure you want to delete Client User? This action cannot be undone.
  - button "Cancel"
  - button "Confirm"
```
